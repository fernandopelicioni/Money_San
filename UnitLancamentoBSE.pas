...
if (vertical <> 1024) or (horizontal <> 768) then MessageBox(handle,'Este sistema foi Desenvolvido para rodar com a Resolução 1024 X 768, É aconselhável Alterar a Resolução','Problemas de Resolução', MB_OK + MB_ICONInformation);
...
//                           Panel1.SetFocus;
                    ...
                    Else
                    Begin
                           MessageBox(handle,'Linha não Encontrada, pertence a outra Coligada ou não está ativa !','Atenção !!!', MB_IconInformation + MB_OK);
                           editLinha.text:=' ';
                           EditLinha.SetFocus;
                    End;
         ...
         with QrMovBcd do
              begin
                   if EditBcd.text <> '' then
                       begin
//  ********************************** Ajuste devido a mudança de numeração do sistema **********
//                            BcdAtual:=EditBcd.Text;
                           if (DtColigada.DataSet.FieldByName('Cod_Coligada').AsInteger = 1)  and (Sistema = '1') and (StrToInt(EditBcd.Text) < 840000) then  // 840000 é o primeiro boletim da numeracao antiga
                                      begin
                                                           ...
                                                           if DataAtual <> Pai.QrBcd.FieldByName('Tbcd_Data').AsString then
                                                                  begin
                                                                      // Se a data do lancamento for diferente do bcd lancado ele pergunta
                                                                       beep;
                                                                       if MessageDlg('A data de Geração do Boletim é diferente da Data do Lançamento !!!, '+#13+'Deseja Lançar mesmo assim?',MtConfirmation,[mbyes,Mbno],0) =mryes then
                                                                              Begin
                                                                              ...
                                                                              Begin
//                                                                                  MessageBox(handle,'Este Boletim Não foi Gerado e por isso Não pode ser Lançado','Boletim Não Gerado / Alocado',MB_OK + MB_ICONStop);
                                                                                    Btcancel.Click;
                                                                                    BitBtn2.Click;
                                                             ...
                                                             begin
                                                                        MessageBox(handle,'Este Boletim Não foi Gerado Ou Está Cancelado, E por isto, não pode ser Lançado','Boletim Não Gerado Ou Cancelado',MB_OK + MB_ICONStop);
                                                                         Btcancel.Click;
                                                                          BitBtn2.Click;
                                                             end;
                                            end;
                     ...
                     begin
                          if Dtparam.DataSet.FieldByName('Tli_Talao').AsString = 'S' then
                                 begin
                                         ...
                                         FormSelecionaTalao.Panel1.Caption:='Escolha ' + FormatFloat('00',pas) + ' Talões !';
                                         FormSelecionaTalao.ShowModal;
                                         if QtTaloes <> pas then
                                                begin
                                                       Msg:='A quantidade de Talões Informados/Utilizados não conferem'+#13+
                                                            'Talões Informados : ' + IntTostr(QtTaloes) +#13+
                                                            'Passageiros : ' + IntToStr(pas);
                                                       MessageBox(handle,Pchar(Msg),'Erro na Quantidade de Talões',MB_OK + MB_ICONStop);
                      ...
                      begin
                            Msg:='Quantitativos Informados não estão corretos, verifique !'+#13 +
                                                                ...
                                                                QrMovConf.FieldByName('MCf_Vpapel').AsString:=DtMov.DataSet.FieldByName('Bcd_VtPapel').AsString;
                                                                 QrMovConf.FieldByName('MCf_PPapel').AsString:=DtMov.DataSet.FieldByName('Bcd_PePapel').AsString;
                                                                  ...
                                                                  QrMovConf.FieldByName('MCf_Outros').AsString:=DtMov.DataSet.FieldByName('Bcd_Outros').AsString;
                                                                   QrMovConf.Post;
                                                                    QrMovConf.Close;
//*************************************** Salva Débito caso esteja habilitado *****************************************************************************************************************
...
//************************************** Pergunta se usuario quer imprimir recibo, Caso Opcao Esteja marcada em FormDadosMov.ChkRecibo *********************************************************
                                                                  if FormDadosMov.ChkRecibo.Checked = True then
                                                                       begin
                                                                             if messageDlg('Atenção ! '+#13+'O Comprovante está salvo...' +#13+'Deseja Imprimi-lo agora ?',mtConfirmation,[mbyes,mbno],0) = mryes then
                                                                                   begin
//                                                                                  if Dtparam.DataSet.FieldByName('Par_GeraRecibo').AsString = 'I' then
//                                                                                      begin
//                                                                                         Relatorio.RvSystem1.DefaultDest:='rdPrinter';
                                                                                         MessageBox(handle,'Prepare a Impressora para a Emissão do Recibo !','Atenção !',Mb_IconInformation+Mb_Ok);
                                                                 //                     end;
                                                                                                            ...
                                                                                                            Sql.Add('And Mcf_Bcd = ' + QuotedStr(BcdAtual));
                                                                                                             Open;
                                                                                                              Relatorio.Projeto.SetParam('Versao',FormIniciaSistema.EditVersao.Caption);
                                                                                                                 if EditDebito.Text <> '' then Relatorio.Projeto.SetParam('InfoDebito','* Atenção, Você deve : ' + EditDebito.text + ' na Arrecadação!');
...
//--************************************************* Pergunta se quer imprimir o Debito agora ************************************************
                                                                   if FormDadosMov.ChkDebito.Checked = True then
                                                                       begin
                                                                             if EditDebito.Text <> '' then
                                                                                   begin
                                                                                         if messageDlg('Atenção ! '+#13+'O Sistema Criou um Débito para esta Colaborador...' +#13+'Você pode Imprimi-lo para que ele assine'+#13'+Quer fazer isto agora ?',mtConfirmation,[mbyes,mbno],0) = mryes then
                                                                                               begin
                                                                                                     MessageBox(handle,'Prepare a Impressora para a Emissão do Recibo !','Atenção !',Mb_IconInformation+Mb_Ok);
...
//                   showmessage('valor : '+ FormatFloat('0.0000',valor));
                   if valorDigitado > Valor then
                          Begin
                                  ShowMessage('Valor entregue não pode ser maior que o que consta no Boletim, Acerte'+#13+FormatFloat('###0.00',ValorDigitado)+#13+FormatFloat('###0.00',Valor));
                                  EditDinheiro.SetFocus;
                          End
                        else
                          begin
                              if (valor-valordigitado) > 0 then
                                     begin
                                           if messageDlg('Atenção !'+#13+#13+'O valor entregue não é igual ao Valor do Boletim !'+#13+'Deseja Salvar o recibo e Confeccionar o Documento de Débito? ',mtConfirmation,[mbyes,mbno],0) = mryes then
                                                  begin
                                                        if FormDadosMov.ChkDebito.Checked = true then
                                                              begin
                                                                     GroupBox3.Enabled:=true;
...
if KEY = #13 then
       BEGIN
             KEY:=#0;
               try
               if messageDlg('Confirmar Valor do Débito Informado ? ',mtConfirmation,[mbyes,mbno],0) = mryes then
                    begin
                           EditFuncionario.Enabled:=true;
                            EditFuncionario.SetFocus;
                    end
...
//***************************************** HABILITA LANCAMENTO DE VALES ***********************************************
                If FormDadosMov.ChkDebito.Checked = FALSE then
                   Begin
                        LabelDebito.Caption:='Não Permite Criar Débito';
                         LabelDebito.Font.Color:=ClRed;
                          GroupBox3.Enabled:=false;
                   End
                   else
                   begin
                         LabelDebito.Caption:='Permite Criação de Débito';
