             ...
             Close;
              Sql.Clear;
               Sql.Add('select A.Bcd_Vei, max(A.Bcd_CatFin) from tb_MonMov A');
                Sql.Add('Where A.Bcd_Data = (select Max(B.Bcd_data) from tb_MonMov B where b.Bcd_Vei = A.Bcd_Vei and b.BCD_COLIGADA = a.BCD_COLIGADA)');
                 Sql.Add('And A.Bcd_Coligada = ' + DtColigada.DataSet.FieldByName('cod_coligada').AsString);
