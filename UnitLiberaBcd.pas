                 ...
                 Sql.Clear;
                  Sql.Add('Select * from tb_MonBcd where Tbcd_Coligada = '+ QuotedStr(Pai.QrColigadas.FieldByName('cod_coligada').AsString));
                   Open;
                     ...
                     ShowMessage(IntToStr(X)+' Foram Gerados com sucesso e ja podem ser usados na prestação de contas...');
          ...
          with QrPro do
              begin
                    ...
                    if not eof then
                         ...
                         begin
                               ShowMessage('Colaborador não existe ou ja demitido');
                                Edit1.SetFocus;
                         end;
              end;
     end;
                        ...
                        if DtBcd.DataSet.FieldByName('TBcd_status').AsString <> 'F' then
                           begin
                            if MessageDlg('Boletim ja foi emitido, Deseja alterar os Dados?', mtconfirmation,[mbyes,mbno],0) = mryes then
                                ...
                                ShowMessage('Boletim ja foi acertado na arrecadação, não é possivel alterar');
                                 EditBcd.Text:='';
                                  EditBcd.SetFocus;
                           end;
                       end;
