...
//                                                                                        QrConsiste.FieldByName('Rco_Vei').AsString:=QrValida.Fields[0].Value;
                                                                              ...
                                                                              Memo1.Lines.Add('* Veículo ' + QrValida.Fields[0].Value);
                                                                        end;
                                                          end;
                                        End;
                            Next
...
Memo1.Lines.Add('***** Inicio da Verificação Entre BCD´s ******');
           with QrValida do
               Begin
                     Close;
                     Sql.Clear;
                               ...
                               if VeiculoAtual = QrValida.FieldByName('Bcd_Vei').AsString then
                                      begin
                                            if CatracaAnterior <> QrValida.FieldByName('Bcd_CatIni').AsInteger then
                                                 Begin
                                                      Memo1.Lines.Add('* Atenção ,' + QrValida.FieldByName('Bcd_Vei').AsString + ' Catraca Anterior : ' + IntToStr(CatracaAnterior) + ', Atual : ' + QrValida.FieldByName('Bcd_CatIni').AsString + ', BCD : ' + QrValida.FieldByName('Bcd_Num').AsString);
                                                 End;
                                      end;
                                      CatracaAnterior:=QrValida.FieldByName('Bcd_CatFin').AsInteger;
                                      VeiculoAtual:=QrValida.FieldByName('Bcd_Vei').AsString;
               ...
               End;
Memo1.Lines.Add('Validação de Catracas terminado, Observe as linhas com * (Asteriscos)');
