...
begin
      ...
      begin
          if MessageDlg('Confirma Baixa do Débito do Sr(a): ' + DtDebitos.DataSet.FieldByName('Nom_Pro').AsString + ' ?' , mtConfirmation, [mbYes, mbNo], 0) = MrYes then
                begin
...
begin
      if DtDebitos.DataSet.active then
      begin
          if MessageDlg('Confirma Exclusão do Débito do Sr(a): ' + DtDebitos.DataSet.FieldByName('Nom_Pro').AsString + ' ?' , mtConfirmation, [mbYes, mbNo], 0) = MrYes then
                begin
                       with QrApagar do
                           begin
                                Close;
